			...
			<view class="tui-flex-box">
				...
				<view class="tui-addr">
					...
					<view class="tui-Technical" @tap="toTel(orderDetail.orderInfo.Technical.tel)"
						v-if="orderDetail.orderInfo.Technical">
						接单{{lang.technical}}：{{orderDetail.orderInfo.Technical.title}}<text class="tui-addr-tel"
							style="color: #5454f3;">{{orderDetail.orderInfo.Technical.tel}}</text></view>
				</view>
			</view>
		</tui-list-cell>
	...
	export default {
		...
		methods: {
			detail() {
				...
				_this.$request.post('qrcode.timescard', {
					orderid: orderid,
					is_hyaline: true,
					expireHours: 1
				}).then(function(res) {
			...
			toadditionalPayTap: function(total, orderid) {
				...
				uni.showModal({
					...
					success: function(res) {
						const redirectUrl = "/pagesA/my/myOrder/myTimesDetail?id=" + orderid;
			...
			yuyueTime() {
				this.tui.href("/pagesA/my/myOrder/yuyuetime?id=" + this.orderDetail.orderInfo.id)
			},
			refund() {
