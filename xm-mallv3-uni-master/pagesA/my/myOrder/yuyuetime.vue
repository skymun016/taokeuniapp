<template>
	<view class="content">
		<tui-tabs :top="0" :tabs="tabs" :currentTab="timestype" itemWidth="50%" @change="change"></tui-tabs>
		...
		methods: {
			...
			handleSubmit() {
				...
				if (_this.timestype == 1) {
					yue_date = _this.pickerMonth;
				} else {
					yue_date = _this.pickerWeeks + 1;
				}
						...
						uni.showModal({
							...
							success: function(res) {
								if (res.cancel) { //点击取消,默认隐藏弹框
									...
									_this.tui.href('/pagesA/my/myOrder/myTimes');
								}
							}
						});
