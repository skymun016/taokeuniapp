	...
	<view>
		<view class="tui-mybg-box">
			<view class="tui-header-center">
				<image :src="partnerinfo.touxiang || '/static/images/my/mine_def_touxiang_3x.png'" class="tui-avatar"
					@tap="navigateTo('/pagesA/my/userInfo/userInfo')"></image>
				<view class="tui-info">
		...
		<view class="tui-content-box">
			<view class="tui-box tui-assets-box">
				<tui-list-cell padding="0" unlined :hover="false">
					<view class="tui-cell-header">
						<view class="tui-cell-title">我的分红</view>
					</view>
				</tui-list-cell>
				<view class="tui-order-list tui-assets-list">
					<view class="tui-order-item">
						<view class="tui-assets-num"><text>{{partnerinfo.total_income}}</text></view>
						<view class="tui-assets-text">总收入</view>
					</view>
					<view class="tui-order-item">
						<view class="tui-assets-num">
							<text>{{partnerinfo.income}}</text>
						</view>
			...
			<view class="tui-box tui-tool-box">
				<view class="tui-order-list tui-flex-wrap">
					<view class="tui-tool-item" @tap="navigateTo('/pagesA/my/partner/commisionlog')">
						<view class="tui-icon-box">
							<image src="/static/images/mall/my/icon_ticket_3x.png" class="tui-tool-icon"></image>
						</view>
						<view class="tui-tool-text">分红明细</view>
					</view>
					<view class="tui-tool-item" @tap="navigateTo('/pagesA/my/partner/teamOrder')">
						<view class="tui-icon-box">
							<image src="/static/images/mall/my/icon_kaipiao_3x.png" class="tui-tool-icon"></image>
						</view>
						<view class="tui-tool-text">分红订单</view>
					</view>
					<view class="tui-tool-item" @tap="navigateTo('/pagesA/my/withdraw/withdrawlog?mo=partner')">
						<view class="tui-icon-box">
							<image src="/static/images/mall/my/icon_zhihuan_3x.png" class="tui-tool-icon"></image>
						</view>
						<view class="tui-tool-text">提现记录</view>
					</view>
					<view v-if="partnerinfo.is_submitaudit!=1" class="tui-tool-item" @tap="navigateTo('/pagesA/my/withdraw/withdraw?mo=partner')">
						<view class="tui-icon-box">
							<image src="/static/images/mall/my/icon_fanxian_3x.png" class="tui-tool-icon"></image>
						</view>
						<view class="tui-tool-text">提现</view>
					</view>
					<view v-if="partnerinfo.is_submitaudit!=1" class="tui-tool-item" @tap="navigateTo('/pagesA/my/memberbankcard/memberbankcard')">
						<view class="tui-icon-box">
								...
								<view class="tui-pro-content">
									...
									<view>
										<view class="tui-pro-price">
											<text v-if="item.is_points_goods==1" class="tui-sale-price">{{lang.points}}:{{ item.pay_points }}</text>
...
<script>
	export default {
		...
		onShow() {
			let _this = this;
			_this.$request.get('partner.check', {
						...
						uni.showModal({
						  ...
						  success: function (res) {
						    if (res.cancel) {//点击取消,默认隐藏弹框
								...
								_this.sam.navigateTo("/pages/login/reg?ptype=partner");
						    }
						  }
						});
					}else{
						_this.partnerinfo = res.data;
					}
				}
			});
		},
