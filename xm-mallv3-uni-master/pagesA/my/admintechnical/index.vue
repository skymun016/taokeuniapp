<template>
	<view v-if="is_login==1">
		<view class="technicalbox">
			<view class="tui-mybg-box">
				<view class="tui-header-center">
					<image :src="technical.touxiang || '/static/images/my/mine_def_touxiang_3x.png'" class="tui-avatar">
					</image>
					<view class="tui-info">
					...
					<view class="tui-btn-edit">
						<!--营业-->
						<view class="tui-btn-text">{{ technical.is_business == 1 ? '上班中' : '休息中' }}</view>
						<tui-button type="warning" shape="circle" width="92rpx" height="40rpx" :size="22"
							@click="business(technical.is_business)">{{ technical.is_business == 1 ? '休息' : '上班' }}
						</tui-button>
					</view>
				</view>
			</view>
			<view class="tui-content-box">
				...
				<view class="tui-box tui-order-box" style="margin-top: 20rpx;">
					<tui-list-cell :arrow="true" padding="0" unlined :lineLeft="false"
						@click="navigateTo('/pagesA/my/admintechnical/order?ptype=2')">
					...
					<view class="tui-order-list">
						<view v-if="technicalconfig.is_pickuporder == 1" class="tui-order-item"
							@tap="navigateTo('/pagesA/my/publicOrder/order?identity=technical')">
							<view class="tui-icon-box">
								<tui-icon name="clock" :size="30" color="#ff1e02"></tui-icon>
							</view>
							<view class="tui-order-text">任务大厅</view>
						</view>
						<view class="tui-order-item"
							@tap="navigateTo('/pagesA/my/admintechnical/order?currentTab=1&ptype=2')">
							<view class="tui-icon-box">
						...
						<view class="tui-order-item"
							@tap="navigateTo('/pagesA/my/admintechnical/order?currentTab=2&ptype=2')">
							<view class="tui-icon-box">
						...
						<view class="tui-order-item"
							@tap="navigateTo('/pagesA/my/admintechnical/order?currentTab=4&ptype=2')">
							<view class="tui-icon-box">
					...
					<tui-list-view>
						...
						<tui-list-cell @tap="qrcode" :arrow="true">
							<view class="tui-item-box">
								...
								<view class="tui-list-cell_name">我的二维码</view>
							</view>
						</tui-list-cell>
						<tui-list-cell v-if="technical.is_submitaudit != 1"
							@tap="navigateTo('/pagesA/my/withdraw/withdraw?mo=technical')" :arrow="true">
							<view class="tui-item-box">
								<tui-icon name="wealth-fill" :size="24" color="#5677fc"></tui-icon>
								<text class="tui-list-cell_name">申请提现</text>
							</view>
						</tui-list-cell>
						<tui-list-cell @tap="navigateTo('/pagesA/my/withdraw/withdrawlog?mo=technical')" :arrow="true">
							<view class="tui-item-box">
						...
						<tui-list-cell @tap="navigateTo('/pagesA/my/forhelp/forhelp')" :arrow="true">
							<view class="tui-item-box">
								<tui-icon name="voipphone" :size="24" color="#ff1e02"></tui-icon>
								<view class="tui-list-cell_name">紧急求助</view>
							</view>
						</tui-list-cell>
						<!-- #ifdef MP-WEIXIN -->
						...
						<!-- #endif -->
						<tui-list-cell @tap="navigateTo('/pagesA/my/admintechnical/update')" :arrow="true">
							<view class="tui-item-box">
								<tui-icon name="setup" :size="24" color="#5677fc"></tui-icon>
...
<script>
	export default {
		...
		onShow() {
			...
			_this.$request.get('admintechnical.check', {
				...
				if (res.errno == 0 && res.is_login == 1) {
					...
					_this.technicalconfig = _this.technical.Config;
				} else {
					uni.showModal({
						title: '提示',
						content: '请先登录！',
						...
						success: function(res) {
							if (res.cancel) { //点击取消,默认隐藏弹框
								...
								uni.reLaunch({
									url: "/pages/login/userlogin?ptype=technical",
								})
							}
						}
					});
		...
		methods: {
			...
			business() {
				let _this = this;
				var confirmText = '确定休息吗？';
				if (_this.technical.is_business == 0) {
					confirmText = '确定上班吗？';
				}
				uni.showModal({
					title: '提示',
					...
					success: function(res) {
						if (res.confirm) {
							_this.$request.post('admintechnical.business', {
								id: _this.technical.id,
								samkey: (new Date()).valueOf()
							}).then(res => {
								if (res.errno == 0) {
									uni.showToast({
			...
			async qrcode() {
				...
				let qrdata = await _this.$request.post('qrcode.technical', {
					id: _this.technical.id,
					page: '',
					is_hyaline: true,
					expireHours: 1
