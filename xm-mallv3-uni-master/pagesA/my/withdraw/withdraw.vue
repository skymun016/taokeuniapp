		...
		<form @submit="bindSave" :report-submit="true">
			<tui-list-cell :hover="false" :lineLeft="false">
				<view class="tui-cell-input">
					<input name="amounts" @input="inputamounts" :placeholder="'提现金额,最少'+min_money+'元'" placeholder-class="tui-phcolor"
	...
	export default {
		...
		methods: {
			chooseMemberbankcard() {
				uni.navigateTo({
					url: "/pagesA/my/memberbankcard/memberbankcard"
				})
			},
			inputamounts (e) {
				let _this = this;
				_this.$request.post('withdraw.calculate', {
					mo: _this.mo,
					amounts: e.detail.value
				}).then(res => {
					if (res.errno == 0) {
						_this.actual_amounts = res.data.actual_amounts;
						_this.poundage = res.data.poundage;
					}
				})
			},
