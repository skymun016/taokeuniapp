			...
			<view class="tui-order-item" v-for="(item,orderIndex) in orderList" :key="orderIndex">
				...
				<view class="tui-order-btn">
					<view class="tui-btn-ml">
						<tui-button @click="orderDetail(item.id)" type="black" plain width="152rpx" height="56rpx" :size="26"
							shape="circle">订单详情</tui-button>
					</view>
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		onLoad: function(e) {
			...
			_this.$request.get('orderstatus.listname',{ptype:'service'}).then(res => {
				if (res.errno == 0) {
		...
		methods: {
			...
			getorderlist: function(isPage) {
				...
				//console.log(_this.where.status);
				_this.$request.post('order.adminorder', _this.where).then(res => {
					if (res.errno == 0) {
			...
			orderDetail: function(orderid) {
				var url = '/pagesA/my/admin/orderDetail?id='+orderid;				
				uni.navigateTo({
					url: url
				})
			},
