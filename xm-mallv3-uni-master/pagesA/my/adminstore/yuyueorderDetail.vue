<template>
	<view class="container">
		...
		<tui-list-cell v-if="orderDetail.orderInfo.deliverymode==1 || orderDetail.orderInfo.deliverymode==3" unlined
			...
			<view class="tui-flex-box">
				...
				<view class="tui-addr">
					...
					<view class="tui-Technical" @tap="toTel(orderDetail.orderInfo.Technical.tel)"
						v-if="orderDetail.orderInfo.Technical">接单{{lang.technical}}：{{orderDetail.orderInfo.Technical.title}}<text
							class="tui-addr-tel" style="color: #5454f3;">{{orderDetail.orderInfo.Technical.tel}}</text>
					</view>
				</view>
			</view>
		</tui-list-cell>
		...
		<view class="tui-order-item">
			...
			<diyfields ptype="complete" :orderid="orderid" gourl="no" submittxt="提交备注" posturl='order.custom'>
			</diyfields>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			staff() {
				var url = '/pagesA/my/adminstore/selectTechnical?id=' + this.orderDetail.orderInfo.id + '&sid=' + this
					.orderDetail.orderInfo.sid;
				uni.navigateTo({
					url: url
				})
			},
