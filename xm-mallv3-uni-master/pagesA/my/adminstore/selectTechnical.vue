		...
		<scroll-view class="scrollList" scroll-y :scroll-into-view="scrollViewId" :style="{height:winHeight+'px'}">
			<view class="search-bar">
				<view class="search-bar-form">
					<view class="search-bar-box">
						...
						<input confirm-type="search" class="search-bar-input" placeholder="输入关键词"
					...
					<label class="search-bar-label" v-if="!inputShowed" @tap="showInput">
						...
						<view class="search-bar-text">输入关键词</view>
					</label>
				</view>
			</view>
			<view class="tui-list search-result" v-if="inputShowed">
				<view class="tui-list-cell" hover-class="tui-list-cell-hover" v-for="(item,index) in searchResult"
					:key="index" @tap="selectTechnical" :data-uuid="item" :hover-stay-time='150'>
			...
			<view v-if="!inputVal">
				<view class="tui-list city-list" >
					...
					<view class="tui-list-cell" hover-class="tui-list-cell-hover" v-for="(item,index) in lists"
						:key="index" @tap="selectTechnical" :data-uuid="item.uuid" :hover-stay-time='150'>
						<view class="tui-list-cell-navigate">
							{{item.title}}
						</view>
					</view>
					...
					<block v-else>
						<view style="text-align: center;color: #999;">您的店铺还没有{{lang.technical}}！</view>
					</block>
				</view>
			</view>
		</scroll-view>
...
<script>
	export default {
		...
		onLoad: function(options) {
			...
			_this.$request.post('technical.list', {
				sid: _this.sid
			}).then(res => {
