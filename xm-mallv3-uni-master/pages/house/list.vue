<template>
	<view class="container">
		...
		<!--list-->
		<view class="tui-data-list" :class="[isList ? 'tui-flex-list' : 'tui-flex-card']">
			<view class="tui-data-container">
				<block v-for="(item, index) in goodsList" :key="index">
					<!--商品列表-->
					<view class="tui-data-item" hover-class="tui-hover" :hover-start-time="150"
						:data-url="'/pages/house/detail?id=' + item.id" @tap="navigationTo">
						<image :src="item.image" class="tui-pro-img" :class="[isList ? 'tui-proimg-list' : '']"
							mode="widthFix" />
						<view class="tui-item-content">
							<view class="tui-item-tit">{{ item.name }}</view>
							<view class="tui-item-body">
								<view><text>{{item.room}}室{{item.hall}}厅/{{item.floorspace}}平米/{{item.toward}}</text></view>
								<text class="tui-sale-price">￥{{ item.price }} 万</text>
							</view>
						</view>
					</view>
					<!--商品列表-->
	...
	export default {
		...
		onLoad: function(options) {
			...
			uni.getSystemInfo({
				success: res => {
					...
					this.height = obj.top ? obj.top + obj.height : res.statusBarHeight;
					//略小，避免误差带来的影响
					this.dropScreenH = this.height;
		...
		methods: {
			...
			get_data_list: function(isPage) {
				...
				this.$request.post('house', that.where).then(res => {
					if (res.errno == 0) {
