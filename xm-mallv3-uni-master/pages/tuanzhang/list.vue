					...
					<view @tap="selectTap" :data-item="item" class="item">
						...
						<view class="text">
							...
							<view class="indicators">
								<text class="indctext">地址: {{item.city_name}}{{item.district_name}}{{item.region_name}}{{item.house_number}}</text>
							</view>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//查找
			get_tuanzhang_list: function(isPage) {
				var that = this;
				this.setWhere();
				...
				that.$request.post('tuanzhang.list', that.where).then(res => {
