<template>
	<view class="container">
		<view class="no-goods" v-if="goodsList.list.length<1">
			...
			<view class="to-index-btn" :style="'background:'+ pagestyleconfig.appstylecolor" @tap="toIndexPage">
				去逛逛
			</view>
		</view>
		<view v-if="goodsList.list.length>0">
			...
			<view class="settlement">
				...
				<view>				
					<button @click="toPayOrder" :disabled="goodsList.noSelect" v-if="goodsList.saveHidden" class="settlementbutt" :style="'background:'+ pagestyleconfig.appstylecolor">去结算({{goodsList.number}})</button>
					<tui-button @click="deleteSelected" :disabled="goodsList.noSelect" width="200rpx" height="70rpx"
						:size="30" type="black" shape="circle" :plain="true" v-else>
						删除</tui-button>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			navigateToPayOrder: function() {
				uni.hideLoading();
				this.sam.navigateTo('/pagesA/submitOrder/submitOrder');
			}
		},
		/**
		 * 页面相关事件处理函数--监听用户下拉动作
...
<style>
	...
	.title-box {
		...
		font-size: 28rpx;
		color: #999;
		background: url(/static/images/icon-cart.png) no-repeat center 205rpx;
