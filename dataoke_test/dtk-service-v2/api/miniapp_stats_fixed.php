...
try {
    ...
    // 获取转链统计（如果表存在）
    $convertStats = ['today' => ['total' => 0, 'success' => 0, 'success_rate' => 0], 'total' => ['total' => 0, 'success' => 0, 'success_rate' => 0]];
    try {
        $convertSql = "SELECT COUNT(*) as count FROM dtk_convert_logs LIMIT 1";
        $stmt = $db->prepare($convertSql);
        $stmt->execute();
        ...
        $convertStats['today'] = [
            ...
            'success_rate' => intval($todayConvert['total']) > 0 ? round(intval($todayConvert['success_count']) / intval($todayConvert['total']) * 100, 1) : 0
        ];
    } catch (Exception $e) {
        // 表不存在，使用默认值
    }
