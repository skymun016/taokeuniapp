...
use app\model\MiaoshaGoodsSkuValue;
...
class MiaoshagoodsController extends Base
{
	...
	function index()
	{
		...
		$query = MiaoshaGoods::where(['weid' => $weid]);
		if (!empty($keyword)) {
	...
	public function update()
	{
		...
		if (empty($id)) {
			...
			try {
				$res = MiaoshaGoods::create($data);
				if ($res->id && empty($data['sort'])) {
					MiaoshaGoods::update(['sort' => $res->id, 'id' => $res->id]);
				}
				$data['id'] = $res->id;
	...
	function _synupdata($data)
	{
		...
		MiaoshaGoodsSkuValue::where(['ms_id' => $data['id'], 'goods_id' => $data['goods_id']])->delete();
		if (isset($data['sku'])) {
			foreach ($data['sku'] as $skuarr) {
				MiaoshaGoodsSkuValue::create([
					'ms_id' => (int) $data['id'],
	...
	function getInfo()
	{
		...
		$data = MiaoshaGoods::field('*')->find($id)->toArray();
		...
		$data['sku'] = MiaoshaGoodsSkuValue::field('sku,image,quantity,price')->where(['ms_id' => $id, 'goods_id' => $data['goods_id']])
			->order('id asc')
			->select()->toArray();
	...
	function delete()
	{
		return $this->del(new MiaoshaGoods());
	}
}
